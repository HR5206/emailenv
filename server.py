...
from models import Action, Observation, State, Reward, ResetResponse, EmailTask, EnvState, AgentAction
...
async def reset(body: Optional[ResetRequest] = Body(None)):
    ...
    try:
        ...
        return ResetResponse(
            ...
            available_tasks=["spam_classification", "email_prioritization", "reply_generation"],
        )
...
async def step(body: Action | StepRequest = Body(...)):
    ...
    try:
        ...
        logger.info(f"[STEP] step=unknown action=unknown reward=0.00 done=false error={str(e)}")
        raise
